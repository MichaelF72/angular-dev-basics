...
@Component({
  selector: 'app-kapitel-04-dependency-injection',
  standalone: true,
  template: `
    <h1>{{ data }}</h1>
    <!-- Anzeige der Nachricht vom Service -->
    <h2>{{ token }}</h2>
    <!-- Anzeige des statischen Wertes vom Token -->
  `,
...
export class Kapitel04DependencyInjectionComponent {
  data: string;
  token: string;
